...
export async function matchChain(deviceId, userInput) {
  const result = await db.execute({
    sql: `SELECT * FROM chains WHERE device_id = ? AND enabled = 1`,
    args: [deviceId],
  });
  ...
  for (const chain of result.rows) {
    ...
    // Exact match or input contains the trigger phrase
...
export async function getActiveChainState(sessionId) {
  const result = await db.execute({
    sql: `SELECT * FROM chain_state
          WHERE session_id = ?
            AND status IN ('running', 'awaiting_confirm')
...
export async function startChain(sessionId, chain) {
  ...
  const stateResult = await db.execute({
    sql: `INSERT INTO chain_state (session_id, chain_id, current_step, status, context, steps_run, steps_skipped)
          VALUES (?, ?, 0, 'running', '{}', '[]', '[]')
          RETURNING *`,
    args: [sessionId, chain.id],
  });
...
export async function resumeChain(sessionId, chainStateId, action) {
  const stateResult = await db.execute({
    sql: `SELECT * FROM chain_state WHERE id = ? AND session_id = ?`,
    args: [chainStateId, sessionId],
  });
  const state = stateResult.rows[0];
...
async function advanceChain(state, steps, sessionId) {
  ...
  while (currentState.current_step < steps.length) {
    ...
    if (step.step_type === 'silent') {
      const actionResult = await buildAction(step);
...
async function buildAction(step) {
...
async function reloadState(stateId) {
  const result = await db.execute({
    sql: `SELECT * FROM chain_state WHERE id = ?`,
    args: [stateId],
  });
...
async function advanceStep(stateId, nextStep) {
  await db.execute({
    sql: `UPDATE chain_state SET current_step = ?, status = 'running', updated_at = datetime('now') WHERE id = ?`,
    args: [nextStep, stateId],
  });
}

async function markState(stateId, status, reason) {
  await db.execute({
    sql: `UPDATE chain_state SET status = ?, abort_reason = ?, updated_at = datetime('now') WHERE id = ?`,
    args: [status, reason || null, stateId],
  });
}
...
async function updateContext(stateId, patch) {
  ...
  await db.execute({
    sql: `UPDATE chain_state SET context = ?, updated_at = datetime('now') WHERE id = ?`,
    args: [JSON.stringify(ctx), stateId],
  });
}
...
async function addRun(stateId, stepId, currentJson) {
  ...
  arr.push(stepId);
  await db.execute({
    sql: `UPDATE chain_state SET steps_run = ?, updated_at = datetime('now') WHERE id = ?`,
    args: [JSON.stringify(arr), stateId],
  });
}
...
async function addSkipped(stateId, stepId, currentJson) {
  ...
  arr.push(stepId);
  await db.execute({
    sql: `UPDATE chain_state SET steps_skipped = ?, updated_at = datetime('now') WHERE id = ?`,
    args: [JSON.stringify(arr), stateId],
  });
}
...
async function abortExistingChains(sessionId, reason) {
  await db.execute({
    sql: `UPDATE chain_state
          SET status = 'aborted', abort_reason = ?, updated_at = datetime('now')
          WHERE session_id = ? AND status IN ('running', 'awaiting_confirm')`,
    args: [reason, sessionId],
  });
}
