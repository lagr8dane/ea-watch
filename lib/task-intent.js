...
export function parseTaskIntent(raw) {
  ...
  m = text.match(/^create (a )?task (to )?(.+)$/i);
  if (m) return parseAdd(m[3]);

  m = text.match(/^make (a )?task (to )?(.+)$/i);
  if (m) return parseAdd(m[3]);

  m = text.match(/^create (a )?task for\s+(.+)$/i);
  if (m) return parseAdd(m[2]);
  ...
  // Natural phrasing with a trailing time word, e.g. "I need to go to the gym tonight"
  m = text.match(/^(i\s+)?(need to|have to|got to|gotta)\s+(.+)\s+(tonight|today|tomorrow)\.?$/i);
  if (m) {
    const body = m[3].trim();
    // Avoid treating "need to know / understand …" as a physical task
    if (/^(know|understand|figure out|find out|learn|ask|wonder)\b/i.test(body)) return null;
    if (/^see if\b/i.test(body)) return null;
    return parseAdd(`${body} ${m[4].toLowerCase()}`);
  }
...
function parseAdd(rest) {
  ...
  if (/\btomorrow\b$/i.test(title)) {
    ...
    dueDate = todayDateKey();
    title = title.replace(/\s*,?\s*tonight\s*$/i, '').trim();
  }
