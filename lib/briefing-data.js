...
function formatLatLonLabel(lat, lon) {
  ...
  const b = Math.abs(lon).toFixed(2);
  return `${a}°${ns}, ${b}°${ew}`;
}

/** City-level label via OSM Nominatim (best-effort; never throws). */
export async function reverseGeocodeLabel(lat, lon) {
  try {
    ...
    const res = await fetch(u.toString(), {
      headers: { 'User-Agent': 'ea-watch/1.0 (https://github.com/lagr8dane/ea-watch)' },
...
export async function fetchWeatherSnapshot(lat, lon) {
  ...
  const [res, placeFromOsm] = await Promise.all([
    fetch(url, { signal: AbortSignal.timeout(5000) }),
    reverseGeocodeLabel(la, lo),
  ]);
  ...
  const coordsLabel = formatLatLonLabel(la, lo);
  const location_label = placeFromOsm || coordsLabel;
