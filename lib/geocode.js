...
export async function enrichItemsWithDistance(items, origin) {
  ...
  async function bestHitForItem(it) {
    const vLat = it.venue_lat;
    const vLon = it.venue_lon;
    if (Number.isFinite(vLat) && Number.isFinite(vLon)) {
      const label =
        (it.geocode_label != null && String(it.geocode_label).trim()) ||
        (it.address_hint != null && String(it.address_hint).trim()) ||
        (typeof it.title === 'string' ? it.title.trim() : '') ||
        '';
      return { lat: vLat, lon: vLon, display_name: label || `${vLat},${vLon}` };
    }
