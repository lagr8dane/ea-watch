...
function mapTaskRow(t) {
  return {
    id: t.id,
    title: t.title,
    due: t.due_at ? t.due_at.slice(0, 10) : null,
    priority: t.priority,
  };
}

/**
 * Payload for EA chat task panel (JSON over SSE).
 * @param {'all'|'overdue'|'today'} filter
 */
export function buildTaskPanelPayload(rows, { filter = 'all', tasksUrl = '/tasks' } = {}) {
  const todayKey = localDateKey(new Date());
  const open = rows.filter((t) => t.status === 'open');

  if (filter === 'overdue') {
    const tasks = open.filter((t) => bucketTask(t, todayKey) === 'overdue').map(mapTaskRow);
    return {
      headTitle: 'Overdue',
      filter: 'overdue',
      groups: tasks.length ? [{ label: null, tasks }] : [],
      tasksUrl,
    };
  }
  if (filter === 'today') {
    const tasks = open.filter((t) => bucketTask(t, todayKey) === 'today').map(mapTaskRow);
    return {
      headTitle: 'Due today',
      filter: 'today',
      groups: tasks.length ? [{ label: null, tasks }] : [],
  ...
  for (const [key, label] of order) {
    const tasks = open.filter((t) => bucketTask(t, todayKey) === key).map(mapTaskRow);
    if (tasks.length) groups.push({ label, tasks });
  }
  return { headTitle: 'Tasks', filter: 'all', groups, tasksUrl };
}
