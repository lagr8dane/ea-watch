 ...
 * Always prefers an explicit device_id from /me/player/devices first — iOS often has no “active” player until then.
 */
export async function startPlayback(accessToken, { trackUri, playlistUri }) {
  ...
  if (res.status === 404) {
    return {
      ...
      message:
        json?.error?.message ||
        'Spotify has no reachable player right now. Open the Spotify app on your phone, press play on any song once, wait a few seconds, then try again in EA.',
    };
  }
  if (res.status === 403) {
