...
/**
 * Start playback. Premium required. Targets explicit device when none is "active" (typical on iOS).
 */
export async function startPlayback(accessToken, { trackUri, playlistUri }) {
  ...
  let { res, json } = await playOn(null);
  if (res.status === 204) return { ok: true };

  if (res.status === 404) {
    const devices = await listDevices(accessToken);
    const id = pickPlaybackDeviceId(devices);
    if (id) {
      await transferPlayback(accessToken, id);
      await new Promise((r) => setTimeout(r, 400));
      ({ res, json } = await playOn(id));
      if (res.status === 204) return { ok: true };
      ({ res, json } = await playOn(id));
      if (res.status === 204) return { ok: true };
    }
  ...
  if (res.status === 404) {
    return {
      ...
      message:
        json?.error?.message ||
        'No Spotify device available. On iPhone, open Spotify, leave it in the foreground once, or tap Play there — then try again.',
    };
  }
  if (res.status === 403) {
